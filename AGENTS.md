...
- Do not gate hydration *and* an in-view check on visibility at once. `client:visible` already means "has scrolled into view", so a second visibility check inside the component resolves in the same batch and the animation is skipped. When a component should be alive early but animate later, pair `client:idle` with an in-view check (see `Stat.tsx`).
